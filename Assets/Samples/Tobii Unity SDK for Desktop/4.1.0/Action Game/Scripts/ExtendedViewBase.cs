 ...
 */
	...
	public Image OptionalReturnToCenterStimuli;
		...
		public AnimationCurve AmountOfScreenShiftDependingOnCameraPitch =
			AnimationCurve.EaseInOut(0.0f, 1.0f, 1.0f, 1.0f);
		...
		{
			get { return Time.deltaTime < 1E-08f; }
		}
		...
		protected virtual void LateUpdate()
		{
			...
			else
			{
				...
				if (!ShouldPauseExtendedViewOnCleanUi())
				{
					...
					{
						var normalizedCenteredGazeCoordinates =
							CreateNormalizedCenteredGazeCoordinates(_lastValidGazePoint);
						if (!IsLocked)
						{
		...
		private void UpdateHeadPose()
		{
			...
			{
				HeadRotationScalar = !IsAiming
					? Mathf.Lerp(HeadRotationScalar, 1, Time.unscaledDeltaTime * ResetResponsiveness)
					: 0;
			}
			...
			_accumulatedTimeDeltaForHeadPoseLerp =
				Mathf.Clamp(_accumulatedTimeDeltaForHeadPoseLerp, 0.0f, 1.0f / HeadTrackingFrequency);
			...
			if (!HasRecentGazePointData())
			{
				_currentHeadViewSensitivityScale = Mathf.Lerp(_currentHeadViewSensitivityScale,
					HeadViewSensitivityScaleHeadOnly,
					10 * Time.unscaledDeltaTime);
			}
			else
			{
				_currentHeadViewSensitivityScale = Mathf.Lerp(_currentHeadViewSensitivityScale,
					HeadViewSensitivityScale,
					10 * Time.unscaledDeltaTime);
			}
		...
		protected void ProcessAimAtGaze(Camera mainCamera)
		{
			if (!_lastIsAiming && IsAiming && HasRecentGazePointData())
			{
				...
				var aimTargetWorld =
					mainCamera.ScreenToWorldPoint(new Vector3(AimTargetScreen.x, AimTargetScreen.y, 10));
		...
		public void Rotate(Component componentToRotate, float fovScalar = 1f, Vector3 up = new Vector3(),
			bool calculateClampedValues = true)
		{
			...
			if (calculateClampedValues)
			{
				...
				_clampedPitch = _clampedPitch < MaximumRotationLocalAngles.x
					? _clampedPitch
					: MaximumRotationLocalAngles.x;
				_clampedPitch = _clampedPitch > MinimumRotationLocalAngles.x
					? _clampedPitch
					: MinimumRotationLocalAngles.x;
				...
				_clampedPitch =
					Mathf.DeltaAngle(0, _clampedPitch + transformToRotate.rotation.eulerAngles.x) <
					MaximumRotationWorldAnglesX
						? _clampedPitch
						: MaximumRotationWorldAnglesX - transformToRotate.rotation.eulerAngles.x;
				_clampedPitch =
					Mathf.DeltaAngle(0, _clampedPitch + transformToRotate.rotation.eulerAngles.x) >
					MinimumRotationWorldAnglesX
						? _clampedPitch
						: MinimumRotationWorldAnglesX - transformToRotate.rotation.eulerAngles.x;
			}
		...
		public Quaternion GetRotation(Component componentToRotate, float fovScalar = 1f, Vector3 up = new Vector3(),
			bool calculateClampedValues = true)
		{
		...
		protected void RotateAndGetCrosshairScreenPosition(Camera cameraToRotate, out Vector2 crosshairScreenPosition,
			float fovScalar = 1f, Vector3 up = new Vector3())
		{
			Vector3 vector;
		...
		protected void RotateAndGetCrosshairScreenPosition(Camera cameraToRotate, out Vector3 crosshairScreenPosition,
			float fovScalar = 1f, Vector3 up = new Vector3())
		{
		...
		protected void RotateAndGetCrosshairViewportPosition(Camera cameraToRotate, out Vector2 crosshairScreenPosition,
			float fovScalar = 1f, Vector3 up = new Vector3())
		{
			Vector3 vector;
		...
		protected void RotateAndGetCrosshairViewportPosition(Camera cameraToRotate, out Vector3 crosshairScreenPosition,
			float fovScalar = 1f, Vector3 up = new Vector3())
		{
			var crosshairWorldPosition = cameraToRotate.transform.position + cameraToRotate.transform.forward;
			Rotate(cameraToRotate.transform, fovScalar, up);
		...
		private void UpdateViewTarget(Vector2 normalizedCenteredGazeCoordinates)
		{
			...
			_gazeViewTarget.x = Mathf.Lerp(_gazeViewTarget.x, normalizedCenteredGazeCoordinates.x,
				responsiveness * scale);
			_gazeViewTarget.y = Mathf.Lerp(_gazeViewTarget.y, normalizedCenteredGazeCoordinates.y,
				responsiveness * scale);

			/* If you are at an extreme view angle and quickly want to return to the center,
			 * it is nice to be able to look at a given stimulus (usually the crosshair). */
#if (UNITY_5 || UNITY_4_6_OR_NEWER)
		var returnToCenterStimuliTransform =
 OptionalReturnToCenterStimuli == null ? null : OptionalReturnToCenterStimuli.GetComponent<RectTransform>();

		bool userLookingInCenteringDeadzone =
 returnToCenterStimuliTransform == null || !_lastValidGazePoint.IsValid ? normalizedCenteredGazeCoordinates.magnitude < ViewCenteringDeadzoneSize
...
#else
			bool userLookingInCenteringDeadzone =
				normalizedCenteredGazeCoordinates.magnitude < ViewCenteringDeadzoneSize;
#endif
		...
		private void UpdateExtendedViewAngles()
		{
			...
			{
				_currentGazeViewSensitivityScale = Mathf.Lerp(_currentGazeViewSensitivityScale,
					GazeViewSensitivityScaleGazeOnly, 10 * Time.unscaledDeltaTime);
				_currentGazeViewExtensionAngle = Mathf.Lerp(_currentGazeViewExtensionAngle,
					GazeViewExtensionAngleGazeOnly, 10 * Time.unscaledDeltaTime);
			}
			else
			{
				_currentGazeViewSensitivityScale = Mathf.Lerp(_currentGazeViewSensitivityScale,
					GazeViewSensitivityScale, 10 * Time.unscaledDeltaTime);
				_currentGazeViewExtensionAngle = Mathf.Lerp(_currentGazeViewExtensionAngle, GazeViewExtensionAngle,
					10 * Time.unscaledDeltaTime);
			}
			...
			var deltaVector = new Vector2((targetYaw - _gazeYawOffset) / yawLimit,
				(targetPitch - _gazePitchOffset) / pitchLimit);
			var normalizedDistance = Mathf.Clamp01(deltaVector.magnitude);
			var viewTargetStepSize = _currentGazeViewSensitivityScale * TobiiSensitivityGradient(normalizedDistance,
				GazeSensitivityExponent,
			...
			_gazeYawOffset = Mathf.LerpAngle(_gazeYawOffset, targetYaw,
				(IsAiming || !IsEnabled || !HasRecentGazePointData() ? ResetResponsiveness : viewTargetStepSize) *
		...
		private void UpdateFilteredHeadPose(Quaternion headPose, float timeDeltaInSeconds)
		{
			...
			_filteredHeadPose.y +=
				DampenDelta(DampenHeadPose, yaw * Mathf.Deg2Rad - _filteredHeadPose.y,
					-halfHeadPoseRotationalRangeRadians, halfHeadPoseRotationalRangeRadians) * responsiveNess *
				timeDeltaFactor;
			_filteredHeadPose.x +=
				DampenDelta(DampenHeadPose, pitch * Mathf.Deg2Rad - _filteredHeadPose.x,
					-halfHeadPoseRotationalRangeRadians, halfHeadPoseRotationalRangeRadians) * responsiveNess *
				timeDeltaFactor;
			_filteredHeadPose.z +=
				DampenDelta(DampenHeadPose, roll * Mathf.Deg2Rad - _filteredHeadPose.z,
					-halfHeadPoseRotationalRangeRadians, halfHeadPoseRotationalRangeRadians) * responsiveNess *
				timeDeltaFactor;
		}

		private static float DampenDelta(Func<float, float> dampeningFunction, float delta, float minValue,
			float maxValue)
		{
		...
		private static float TobiiSensitivityGradient(float normalizedValue, float exponent, float inflectionPoint,
			float startPoint, float endPoint)
		{
			...
			return ((1.0f - d) * (Mathf.Pow(b, exponent) / a)) +
			       (d * (1.0f - (Mathf.Pow(c * (1.0f - x), exponent) / c)));
		}

		private static float GetSensititivyGradientValue(float centerNormalizedValue, float exponent,
			float inflectionPoint, float startPoint, float endPoint)
		{
			var sign = Mathf.Sign(centerNormalizedValue);
			return sign * TobiiSensitivityGradient(Mathf.Clamp01(Mathf.Abs(centerNormalizedValue)), exponent,
				inflectionPoint, startPoint, endPoint);
		}
		...
		private float GetGazeViewExtensionAngleDegrees(Vector3 centerNormalizedHeadPose)
		{
			switch (ExtendedViewType)
			{
				...
				{
					var headPoseLength = Mathf.Clamp01(Mathf.Sqrt(
						centerNormalizedHeadPose.y * centerNormalizedHeadPose.y +
						centerNormalizedHeadPose.x * centerNormalizedHeadPose.x));
		...
		private Vector3 GetHeadViewAngles(Vector3 centerNormalizedHeadPose)
		{
			...
			angles.y = GetSensititivyGradientValue(centerNormalizedHeadPose.y, HeadSensitivityExponent,
				           HeadSensitivityInflectionPoint, HeadSensitivityStartPoint, HeadSensitivityEndPoint) *
			           _currentHeadViewSensitivityScale * 180.0f;
			angles.x = GetSensititivyGradientValue(centerNormalizedHeadPose.x, HeadSensitivityExponent,
				           HeadSensitivityInflectionPoint, HeadSensitivityStartPoint, HeadSensitivityEndPoint) *
			           _currentHeadViewSensitivityScale * 180.0f;
